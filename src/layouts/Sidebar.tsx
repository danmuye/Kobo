...
export function Sidebar({ collapsed, onToggle, mobileOpen, onMobileClose }: { collapsed: boolean; onToggle: () => void; mobileOpen: boolean; onMobileClose: () => void }) {
  ...
  return (
    <>
      {mobileOpen && <div className="fixed inset-0 z-40 bg-foreground/40 backdrop-blur-sm lg:hidden" onClick={onMobileClose} aria-hidden />}
      ...
      >
          ...
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            {!collapsed && (
              <div className="overflow-hidden">
                <p className="font-display text-base font-bold leading-none text-sidebar-accent-foreground">Kobo</p>
                <p className="text-[10px] uppercase tracking-widest text-sidebar-foreground/60">Finance</p>
              </div>
            )}
          </div>
          <button
        ...
        <nav className="flex-1 overflow-y-auto scrollbar-thin py-4 px-3 space-y-1">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                {active && <motion.span layoutId="sidebar-active" className="absolute left-0 top-1/2 h-6 w-1 -translate-y-1/2 rounded-r-full bg-sidebar-primary" />}
        ...
        <div className={cn("border-t border-sidebar-border p-4", collapsed && "px-2")}>
          {collapsed ? (
            <div className="grid h-9 w-9 place-items-center rounded-full bg-sidebar-accent text-sidebar-accent-foreground text-xs font-semibold mx-auto">AO</div>
          ) : (
            <div className="flex items-center gap-3">
              <div className="grid h-9 w-9 place-items-center rounded-full bg-sidebar-accent text-sidebar-accent-foreground text-xs font-semibold">AO</div>
